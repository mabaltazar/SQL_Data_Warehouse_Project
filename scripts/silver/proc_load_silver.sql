...
BEGIN
	...
	BEGIN TRY -- Error Handling Procedure
		...
		SELECT 
			...
			CASE WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single' -- Normalization, used friendly user information	
				 WHEN UPPER(TRIM(cst_marital_status)) = 'M' THEN 'Married' -- Normalization, used friendly user information
				 ELSE 'N/A' -- Handles missing data
			END AS cst_marital_status,
			CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female' -- Normalization, used friendly user information
				 WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male' -- Normalization, used friendly user information
				 ELSE 'N/A' -- Handles missing data
			END AS cst_gndr,
			cst_create_date
		...
		SELECT 
			...
			CASE UPPER(TRIM(prd_line)) 
				...
				WHEN 'T' THEN 'Touring'
				ELSE 'N/A'
			END AS prd_line, -- Mapping line codes to descriptive values
			CAST(prd_start_dt AS DATE) AS prd_start_dt, -- Changing data types
			CAST(
				DATEADD(DAY, -1, LEAD(prd_start_dt) OVER (PARTITION BY prd_key ORDER BY prd_start_dt)) AS DATE) 
				AS prd_end_dt --Calculate end date subtract one day before the next start date
		...
		SELECT
			CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid))
				 ELSE cid
			END AS cid, -- Removed NAS prefix
			CASE WHEN bdate > GETDATE() THEN NULL
				 ELSE bdate
			END AS bdate, -- Changed future dates to NULL
		...
		PRINT '==============================================';
		PRINT 'Completed Silver Layer Loading';
